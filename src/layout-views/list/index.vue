...
export default {
  ...
  data() {
    ...
    const listReq = this.$hooks.useRequest(this.getList, {
      defaultParams: () => searchQState.state,
    })
    const pageSearch = this.$hooks.usePageSearch({
    ...
    // 编辑新增
    const writeQState = this.$hooks.useQuickState({
      name: '',
      id: '',
      type: 'add',
    })
    const writeSwitch = this.$hooks.useSwitch()
    const addReq = this.$hooks.useRequest(this.add, {
      defaultParams: () => {
    ...
    const updateReq = this.$hooks.useRequest(this.update, {
      defaultParams: () => {
        const params = writeQState.clone()
        delete params['type']
        return params
    ...
    this.$hooks.useEventOn('hello', () => {
      console.log('hello emit')
    })
    this.$hooks.useEventOff('hello')
    this.$hooks.useEventEmit('hello')

    this.$hooks.useEventOnce('world', () => {
      console.log('world emit')
    })
    this.$hooks.useEventEmit('world')
